...
export function getSdk(
  ...
  return {
    ...
    check(
      variables?: CheckQueryVariables,
      requestHeaders?: GraphQLClientRequestHeaders
    ): Promise<CheckQuery> {
      return withWrapper(
        (wrappedRequestHeaders) =>
          client.request<CheckQuery>(CheckDocument, variables, {
            ...requestHeaders,
            ...wrappedRequestHeaders,
          }),
        "check",
...
export function getSdkWithHooks(
  ...
  return {
    ...
    useCheck(
      variables?: CheckQueryVariables,
      config?: SWRConfigInterface<CheckQuery, ClientError>
    ) {
      return useSWR<CheckQuery, ClientError>(
        genKey<CheckQueryVariables>("Check", variables),
        () => sdk.check(variables),
        config
      );
    },

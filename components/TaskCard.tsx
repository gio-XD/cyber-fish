...
function TaskCard(props: { data: TaskFragment; refresh: () => void }) {
  ...
  const isCommittee = account?.cyberClubPass.role === UserRole.Committee;

  const coundSubmit =
    account?.cyberClubPass.role &&
    account?.cyberClubPass.role !== UserRole.Alien;
  const submitted = typeof data.choice === "boolean";
  console.log("🚀 ~ TaskCard ~ data.choice:", data.choice);

  const { hours, minutes } = useTimer({
    expiryTimestamp: fromUnixTime(data.endTime),
  });

  const doTask = async (p: { agree: boolean; challenge?: boolean }) => {
    toggleIsLoading(true);
    const res = await api.doTask({
      input: {
        taskId: data.id,
        result: p.agree,
        challenge: p.challenge || false,
      },
    });

    if (res.doTask.status === DoTaskResponse_Status.Success) {
      await refresh();
      toast({
        title: "Successfully",
  ...
  return (
    ...
    >
      <Flex className=" flex-col gap-2 items-start">
        {data.taskType === TaskType.BinaryDecision ? (
      ...
      <CardFooter className=" flex flex-col gap-1 justify-between p-0 pt-2">
        {(() => {
          if (
            [TaskStatus.InProgress, TaskStatus.PublicReview].includes(
              data.status
            )
          ) {
            return (
          ...
          if (isCommittee && data.status === TaskStatus.CommitteeReview) {
            return (
              <>
                <LoadingButton
                  className="w-full"
          ...
          if (data.status === TaskStatus.End) {
            return data.wrong > data.right ? (
              <LoadingButton
                disabled
                loading={isLoading}
                onClick={() => doTask({ agree: false })}
        ...
        {data.status === TaskStatus.PublicReview ? (
          <LoadingButton
            className="w-full"
            loading={isLoading}
            onClick={() =>
        ...
        {data.status === TaskStatus.CommitteeReview ? (
          <Tag className=" absolute top-[10px] right-[10px]">
            CommitteeReview 
          </Tag>
        ) : undefined}
